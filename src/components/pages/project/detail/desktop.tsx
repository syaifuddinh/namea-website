        ...
        >
          ...
          <div className="flex gap-8 mt-7">
            ...
            <div>
              ...
              <div className="font-medium text-xs mt-1">{category}</div>
              <div className="font-medium text-xs mt-1">{year}</div>
...
const NavButton = ({
  ...
  <Link
    className={`${type === "next" && "justify-end"} ${
      isDisabled ? "bg-gray100 text-gray cursor-not-allowed" : "bg-dark"
    } text-white text-[10px] font-medium flex items-center h-[40px] w-[150px] gap-1`}
    href={url}
  >
    {type === "prev" && (
      <ArrowLeftIcon className={isDisabled ? "text-gray" : "text-primary"} />
    )}
    <div>{type === "next" ? "NEXT PROJECT" : "PREV PROJECT"}</div>
    {type === "next" && (
      <ArrowRightIcon className={isDisabled ? "text-gray" : "text-primary"} />
    )}
  </Link>
);
